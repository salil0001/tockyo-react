...
export default function ByConfirmationDialog(props) {
  ...
  const buyStock = async (event) => {
    ...
    if (!email) {
      ...
      buyQuantity <= 0 ||
      parseInt(buyQuantity) !== parseFloat(buyQuantity)
    ) {
      alert(`Buy Quantity cannot be ${buyQuantity} but atleast 1`);
    } else if (
      props.currentPrice * Math.pow(1.005, buyQuantity) * buyQuantity >
      wallet
    ) {
      alert("Stock total amount is greater than the wallet price.");
    } else {
      setDisableButton(true);
      const createUser = await fetch(`${PRODUCTIONLINK}api/buyStock`, {
        method: "post",
        headers: {
    ...
    <ConfirmationModal>
      <div className="buy-wrapper-parent ">
        <div className="confirmation-wrapper">
          ...
          <div>
            Qty: {buyQuantity} X Price: {props.currentPrice} ={" "}
            {(buyQuantity * props.currentPrice).toFixed(2)}{" "}
          </div>
        </div>
