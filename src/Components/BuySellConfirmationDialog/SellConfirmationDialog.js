...
export default function SellConfirmationDialog(props) {
  ...
  const sellStock = async (event) => {
    ...
    if (!email) {
      ...
      !(sellQuantity > 0 && availableQuantityBackend >= sellQuantity)
    ) {
      ...
      const createUser = await fetch(`${PRODUCTIONLINK}api/sellStock`, {
        method: "post",
        headers: {
  ...
  useEffect(() => {
    const getSellQuantity = async () => {
      const getQuantity = await fetch(`${PRODUCTIONLINK}api/getSellQuantity`, {
        method: "post",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
    ...
    <ConfirmationModal>
      <div className="sell-wrapper-parent">
        <div className="confirmation-wrapper">
          ...
          <div>
            Qty: {sellQuantity} X Price: {props.currentPrice} ={" "}
            {(sellQuantity * props.currentPrice).toFixed(2)}
          </div>
        </div>
        <form onSubmit={sellStock}>
          <div className="form-wrapper">
            ...
            <div>
              <div>Price</div>
              <div className="backend-data" style={{background:"#f5f5f5"}}>{props.currentPrice}</div>
            </div>
            <div>
              <div>IPO</div>
          ...
          <div className="buy-sell-buttons">
            <button
              ...
              disabled={disableSellButton || availableQuantityBackend === 0}
            >
              SELL{" "}
            ...
            <button
              onClick={() => props.handeToggleSellDialog()}
